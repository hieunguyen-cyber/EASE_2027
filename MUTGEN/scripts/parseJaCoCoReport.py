...
def parse_jacoco_report(html_file):
    ...
    missed_lines = int(total_data[7].text.strip())  # Missed lines
    total_lines = int(total_data[8].text.strip())   # Total lines
    branch_coverage = total_data[4].text.strip()   # Branch coverage (e.g., "100%")
    return str(branch_coverage) + "," +str(1-missed_lines/total_lines) + "," + str(missed_lines) + "," + str(total_lines)

# Example usage:
inputreport = sys.argv[1]
srcfilep = sys.argv[2]
prefix_srcfile = srcfilep.split("/")[-1].split(".java")[0]
classname=extract_class_name_from_file(srcfilep)
report_data = parse_jacoco_report(inputreport)
print(prefix_srcfile+","+str(classname[0])+","+report_data)
