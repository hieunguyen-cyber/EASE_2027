...
class LLMRouter:
    ...
    def _build_completion_params(
        ...
        completion_params = {
            ...
            "stream": streaming,
            "temperature": 0.0,
            "timeout": int(os.getenv("MUTGEN_LLM_TIMEOUT_SECONDS", "900")),
        }
        if (
    ...
    def extract_yaml_from_response(self, response: str) -> dict:
        # remove string between <think> and </think>, including tages
        #response = re.sub(r'<think>.*?</think>', '', response, flags=re.DOTALL)
        #response = response.strip().removeprefix("```yaml").removesuffix("```")
        match = re.findall(r'```(?:yml|yaml)(.*?)```', response, flags=re.DOTALL)
        response = match[0] if match else response
