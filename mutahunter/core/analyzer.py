...
class Analyzer:
    ...
    def _find_blocks_nodes(
        ...
        #    print("  " * depth + f"print: {node.type}: {node.text.decode('utf8')}")
    ...
    def get_test_class_node(self, source_file_path: str) -> List[Any]:
        source_code = self._read_source_file(source_file_path)
        return self._find_blocks_nodes(source_file_path, source_code, ["definition.class"])
    def get_import_nodes(self, source_file_path: str) -> List[Any]:
        source_code = self._read_source_file(source_file_path)
        return self._find_import_nodes(source_file_path, source_code, ["import.declaration"])
