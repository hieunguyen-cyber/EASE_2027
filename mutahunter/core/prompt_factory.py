...
class TestGenerationPrompt:
    def __init__(self):
        ...
        # The upstream repository references analyzer templates which are not
        # included in the package.  The focal-method runner calls the generator
        # directly, but eager loading made prompt construction fail before the
        # first LLM request.  Keep the legacy analyze_code() entry point usable
        # by falling back to the generation prompts instead of loading missing
        # files.
        self.analyzer_system_prompt = self.test_generator_system_prompt
        self.analyzer_user_prompt = self.test_generator_user_prompt
        self.fix_assert_system_prompt = env.get_template(
            "test_generation/fix_assert_system.txt"
        )
        self.fix_assert_user_prompt = env.get_template(
            "test_generation/fix_assert_user.txt"
        )
        self.fix_comp_system_prompt = env.get_template(
            "test_generation/fix_comp_system.txt"
        )
        self.fix_comp_user_prompt = env.get_template(
            "test_generation/fix_comp_user.txt"
        )
